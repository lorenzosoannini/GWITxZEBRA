...
from models.ssfe import SSFEProjector, info_nce_loss, cosine_alignment_loss
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(
        description="GWIT training with pretrained GWIT EEG backbone + SIFE + EEG reconstruction + SSFE + subject-wise splits"
    )
    ...
    parser.add_argument("--recon_loss_type", type=str, default="mse", choices=["mse", "smooth_l1"])

    # SSFE
    parser.add_argument("--use_ssfe", action="store_true")
    parser.add_argument("--ssfe_hidden_dim", type=int, default=256)
    parser.add_argument("--ssfe_out_dim", type=int, default=512)
    parser.add_argument("--grl_lambda_ssfe", type=float, default=1.0)
    parser.add_argument("--lambda_ssfe", type=float, default=1.0)
    parser.add_argument("--lambda_ssfe_sem", type=float, default=1.0,
                        help="Weight for semantic InfoNCE: F_s vs CLIP target.")
    parser.add_argument("--lambda_ssfe_inv", type=float, default=1.0,
                        help="Weight for adversarial invariant InfoNCE: F_i vs CLIP target.")
    parser.add_argument("--lambda_ssfe_anchor", type=float, default=1.0,
                        help="Weight for anchor alignment: F vs CLIP target.")
    parser.add_argument("--ssfe_temperature", type=float, default=0.07,
                        help="Temperature for InfoNCE losses.")
    ...
    if args.use_ssfe and not args.use_precomputed_clip_embeds:
        raise ValueError("--use_ssfe requires --use_precomputed_clip_embeds")
    if args.use_ssfe and not args.use_sife:
        raise ValueError("--use_ssfe currently requires --use_sife so that E_i and E_s are available")
...
def main(args):
    ...
    # Rimane uguale al checkpoint originale: non tocchiamo l'architettura del ControlNet
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                ssfe_loss_sem = torch.tensor(0.0, device=accelerator.device)
                ssfe_loss_inv_branch = torch.tensor(0.0, device=accelerator.device)
                ssfe_loss_anchor = torch.tensor(0.0, device=accelerator.device)
                ...
                if sife is not None:
                    ...
                    ### DEBUG ###
                    # Norms of subject-invariant / subject-specific features
                    E_i_norm = E_i.norm(dim=-1).mean()
                    E_s_norm = E_s.norm(dim=-1).mean()
                    Ei_Es_norm_ratio = E_i_norm / (E_s_norm + 1e-8)

                    # Prediction entropy: higher entropy = less confident classifier
                    ...
                    # Random-guess baseline for K active training subjects
                ...
                # ---------------------------------------------------------
                # SSFE loss (ZEBRA-like):
                #   E_i -> F_s   : semantic branch
                #   E_s -> F_i   : adversarial invariant branch
                #   E   -> F     : anchor branch from full EEG representation
                # ---------------------------------------------------------
                if ssfe_projector is not None:
                    if E_i is None or E_s is None:
                        raise RuntimeError("SSFE requires both E_i and E_s from SIFE.")
                    ...
                    ssfe_loss_sem = info_nce_loss(
                        F_s,
                        clip_target,
                        temperature=float(args.ssfe_temperature),
                    )

                    ssfe_loss_inv_branch = info_nce_loss(
                        F_i,
                        clip_target,
                        temperature=float(args.ssfe_temperature),
                    )

                    ssfe_loss_anchor = cosine_alignment_loss(
                        F_anchor,
                        clip_target,
                    )

                    ssfe_loss = (
                        float(args.lambda_ssfe_sem) * ssfe_loss_sem
                        + float(args.lambda_ssfe_inv) * ssfe_loss_inv_branch
                        + float(args.lambda_ssfe_anchor) * ssfe_loss_anchor
                    )

                    if global_step == 0 and accelerator.is_main_process:
                        accelerator.print(f"[DEBUG] F_s shape: {tuple(F_s.shape)}")
                        accelerator.print(f"[DEBUG] F_i shape: {tuple(F_i.shape)}")
                        accelerator.print(f"[DEBUG] F_anchor shape: {tuple(F_anchor.shape)}")
                ...
                # ---------------------------------------------------------
                # ControlNet + UNet
                # Subject conditioning removed: subject-agnostic diffusion
                # ---------------------------------------------------------
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process and (global_step % args.console_log_every == 0 or global_step == 1):
                    logger.info(
                        ...
                        f"ssfe_sem={ssfe_loss_sem.item():.4f} | "
                        f"ssfe_inv={ssfe_loss_inv_branch.item():.4f} | "
                        f"ssfe_anchor={ssfe_loss_anchor.item():.4f}"
                    )
                ...
                if ssfe_projector is not None:
                    ...
                    log_payload["loss_ssfe_sem"] = ssfe_loss_sem.item()
                    log_payload["loss_ssfe_inv"] = ssfe_loss_inv_branch.item()
                    log_payload["loss_ssfe_anchor"] = ssfe_loss_anchor.item()
                ...
                json.dump(
                    {
                        ...
                        "lambda_ssfe_sem": float(args.lambda_ssfe_sem),
                        "lambda_ssfe_inv": float(args.lambda_ssfe_inv),
                        "lambda_ssfe_anchor": float(args.lambda_ssfe_anchor),
                        "ssfe_temperature": float(args.ssfe_temperature),
                    },
                    f,
                    indent=2,
                )
